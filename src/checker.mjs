...
function customFetch(endpoint, metadata, requestHeaders) {
  return async (input, init = {}) => {
    ...
    if (init.method?.toUpperCase() !== "POST" || !init.body || !metadata) {
      return fetch(requestInput, { ...init, headers });
    }
    try {
      return fetch(requestInput, { ...init, headers, body: JSON.stringify({ ...JSON.parse(init.body), ...metadata }) });
    } catch {
      return fetch(requestInput, { ...init, headers });
    }
  };
...
function createModel(config) {
  ...
  const fetcher = customFetch(config.endpoint, filteredMetadata(config.metadata), config.requestHeaders);
...
function errorMessage(error) {
  ...
  if (body) {
    try { body = JSON.parse(body)?.error?.message || JSON.parse(body)?.message; } catch { body = String(body).slice(0, 160); }
  }
...
export async function checkProvider(config, options = {}) {
  ...
  try {
    ...
    if (streamError) return result(config, "error", latencyMs, pingLatencyMs, errorMessage(streamError));
    ...
    return result(config, "error", null, await pingPromise, errorMessage(error));
  } finally {
    clearTimeout(timer);
  }
}
